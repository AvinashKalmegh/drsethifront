...
import { addMediaCategory, updateMediaCategory } from "./mediaApi";
...
export default function AddCategoryModal({ onClose, onSave, editData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("user_id", adminid || 1); // fallback to 1

    try {
      const response = isEdit
        ? await updateMediaCategory(api, editData.id, formData)
        : await addMediaCategory(api, formData);

      toast.success(`✅ Category ${isEdit ? "updated" : "added"} successfully`);
      onSave(response); // update list
      onClose();
    } catch (error) {
